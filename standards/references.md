...
### 评分规则
按 severity × confidence 组合加权计分：

| 组合 | 权重 |
|------|------|
| HIGH severity + HIGH confidence | -4 分 |
| HIGH severity + MEDIUM confidence | -3 分 |
| MEDIUM severity + HIGH confidence | -2 分 |
| MEDIUM severity + MEDIUM confidence | -1 分 |
| LOW severity（任意 confidence）| -0.5 分 |

从满分 10 分开始扣减，**最低得分为 0 分，不产生负分**。
